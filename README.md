...
프로젝트에는 기본 기능을 테스트할 수 있는 간단한 테스트 스크립트가 포함되어 있습니다.
...
테스트 스크립트는 다음 두 가지 핵심 기능을 테스트합니다:
...
- `.env` 파일에 필수 환경변수가 설정되어 있어야 합니다
  ...
  - `KAMIS_CERT_ID`
  - `OPENAI_API_KEY`
- 인터넷 연결이 필요합니다 (KAMIS API 및 OpenAI API 호출)

### 예상 출력

```
✓ .env 파일 로드: /path/to/kamis_agent/.env
✓ KAMIS_CERT_KEY: 설정됨
✓ OPENAI_API_KEY: 설정됨
...
답변:
2025-10-16 기준 사과(후지) 소매가격은 10kg당 평균 15,000원입니다.
